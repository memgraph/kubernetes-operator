...
- StatefulSet `volumeClaimTemplates` cannot be added or removed on a live cluster, and rebuilding the StatefulSet around its pods deadlocks the roll (the StatefulSet controller only recovers adopted pods in ascending ordinal order, MAIN first). `coreDumps.<role>.enabled` is therefore pinned by a CEL transition rule; read `docs/statefulset-volume-claim-templates.md` before touching any claim template knob.
...
- **External access** (`spec.externalAccess`, `type: LoadBalancer`) is operator-owned on both sides: one LoadBalancer Service shared by the coordinators plus one per data instance (selected by the StatefulSet pod-name label), *and* the announced `bolt_server` of every exposed member. The address is discovered, never declared: external-dns hostname annotation first, then the Service status hostname, then its IP, pod DNS until any exists (`Converged=False/ExternalAddressPending`). `{ordinal}` in per-instance annotation values is substituted with the pod ordinal and CEL requires it on a data hostname annotation. Only the bolt address ever leaves the cluster network; the operator itself always dials coordinators on pod DNS (`resources.CoordinatorEndpoints`), never through the LoadBalancer. External Services follow the data pods the operator *runs* (applied count), are pruned by label once the spec stops describing them, and removing the block reverts every `bolt_server` to pod DNS. The e2e suite gets LoadBalancer addresses from MetalLB (`hack/kind-metallb.sh`, run by `make setup-test-e2e`). Gateway API mode is the planned follow-up PR.
